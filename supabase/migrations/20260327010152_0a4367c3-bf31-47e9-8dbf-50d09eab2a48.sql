...
-- RPC to confirm a regular show order: updates status + creates token
...
BEGIN
  ...
  IF NOT FOUND OR s.is_subscription = true THEN
    -- For membership/subscription, just confirm without creating token
    RETURN jsonb_build_object('success', true, 'type', 'subscription');
  END IF;

  -- For regular shows, create a token
  IF o.user_id IS NULL THEN
    RETURN jsonb_build_object('success', true, 'type', 'regular', 'warning', 'No user_id, token not created');
  END IF;
  ...
  -- Create token linked to user and show
